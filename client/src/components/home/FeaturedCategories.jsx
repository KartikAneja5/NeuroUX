...
import { FiLayout, FiGrid, FiNavigation, FiSliders, FiSquare, FiMousePointer } from 'react-icons/fi';
...
const colorMap = {
  ...
  'buttons': 'from-violet-500/20 to-purple-600/10 border-violet-500/20 text-violet-400',
...
export default function FeaturedCategories() {
  ...
  return (
    <section className="py-24 bg-[#080712] relative overflow-hidden select-none">
      {/* Subtle glow */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-12">
          <div>
            <p className="text-xs font-semibold text-violet-400 uppercase tracking-widest mb-2">Browse Catalog</p>
        ...
        >
          {categories.map((cat) => {
            const key = (cat.categoryKey || cat.id).toLowerCase();
            const Icon = iconMap[key] || FiLayout;
            const colorClass = colorMap[key] || 'from-violet-500/20 to-purple-600/10 border-violet-500/20 text-violet-400';
            return (
              <motion.div key={cat.id} variants={itemVariants}>
                <Link
                  to={`/marketplace?category=${key}`}
                  className={`group flex flex-col items-center p-6 rounded-2xl border bg-gradient-to-br ${colorClass} hover:scale-105 transition-all duration-300 hover:shadow-glow-sm text-center`}
                >
                  <Icon size={26} className="mb-3" />
                  <span className="text-sm font-semibold text-white mb-1">{cat.name}</span>
                  <span className="text-xs text-[#8b7fb5]">{cat.count} {cat.count === 1 ? 'component' : 'components'}</span>
                </Link>
              </motion.div>
            );
          })}
